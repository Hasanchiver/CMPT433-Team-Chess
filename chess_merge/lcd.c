...
void loadNewGameScene( void ){
	...
	c8_drawRect( 0, 190, 100, 290, 0x1C, 1 );
...
void loadRestartScene( void ){
	...
	// BTN Draw
	selectLayer(2);
	drawCircle( 170, 250, 80, 0x00, 0 );

	// Text Draw
	selectLayer(2);
	setTextMode();
	setText90();
	textSetCursor(195,217);
	textTransparent(0x00);
	textWrite("Start");
...
void loadWinScene( int winSide ){
	...
	textWrite( (winSide == 1) ? "AI Win!" : "You Win!" );
...
int gameboard_touchPanelListen( int *row, int *col ){
	...
	if(cali.x < 160 || cali.x > 640){
		...
		if( (cali.x < 100)  && (cali.y > 190 && cali.y < 290) ){
...
void update_gameboard(uint8_t old_table[8][8], uint8_t new_table[8][8]) {
	...
	for(int i = 0; i < 8; ++i){
		...
		for(int j = 0; j < 8; ++j) {
			...
			if( old_table[i][j] == new_table[i][j] ){
				continue;
			}else{

				//Not same

				if(new_table[i][j] == 100){
					clearChessPieceLoc(i, j);
					setMoveMarker(i, j);
					continue;
				}else{

					clearChessPieceLoc(i, j);
					updateChessPieceLoc(i, j, new_table[i][j]);
					setMoveMarker(i, j);
					continue;
				}
...
void setMoveMarker(int row, int col){
	...
	selectLayer(2);
	c8_drawRect( row * 60 + 160, col * 60, row * 60 + 60 + 160, col * 60 + 60, MOVED_COLOR, 1 );
...
void setText90( void ){
	...
	spi_write( 0x22, spi_read(0x22) | 0x10 );




	delay(1);
	spi_write( 0x20, spi_read(0x20) | 0x04 );
